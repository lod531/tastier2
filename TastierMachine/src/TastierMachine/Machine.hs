...
run = do
  ...
  case instructionWord of
	...
	Instructions.LoadIG  -> do
          -- memory mapped control and status registers implemented here
	  let a = smem ! (rtp-1)
          case a of
            0 -> put $ machine { rpc = rpc + 1,
                                 smem = (smem // [(rtp, rpc)]) }
            1 -> put $ machine { rpc = rpc + 1,
                                 smem = (smem // [(rtp, rtp)]) }
            2 -> put $ machine { rpc = rpc + 1,
                                 smem = (smem // [(rtp, rbp)]) }
            _ -> put $ machine { rpc = rpc + 1,
                                 smem = (smem // [(rtp-1, (dmem ! (a-3)))]) }
          run

{-
          run
	Instructions.StoIG   -> do
          -- memory mapped control and status registers implemented here
	  let a = 6 
          case a of
            0 -> put $ machine { rpc = (smem ! (rtp-1)), rtp = rtp - 2 }
            1 -> put $ machine { rpc = rpc + 1, rtp = (smem ! (rtp-1)) }
            2 -> put $ machine { rpc = rpc + 1, rtp = rtp - 2,
                                 rbp = (smem ! (rtp+1)) }
            _ -> put $ machine { rpc = rpc + 1, rtp = rtp - 2,  
                                 dmem = (dmem // [(a-3, 99)]) }
          run
...
-}
    ...
    Instructions.Unary i a ->
      case i of
        ...
        Instructions.StoI    -> do --Store updates a variable in a calling frame
	  let b = smem ! (rtp-2)
          let storeAddr = (followChain 0 a rbp smem) + 4 + b
	  let exprAddr = rtp-1
          put $ machine { rpc = rpc + 1, rtp = rtp - 2,
                          smem = (smem // [(storeAddr, (smem ! exprAddr))]) }
          run
